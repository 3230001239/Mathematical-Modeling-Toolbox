	...
	<div class="bg">
		<div class="m">
			...
			<div class="upload">
				...
				<button class="btn" @click="uploadFilem41">上传文件</button>
				<div style="color: red;">
			...
			<div v-if="uploadResult">
				<p><strong>一致性比率CR:</strong> {{ uploadResult['CR'] }}</p>
				<p><strong>检验结果:</strong> {{ uploadResult['cc'] }}</p>
			</div>
	...
	export default {
		...
		methods: {
			...
			async uploadFilem41() {
				...
				try {
					const response = await axios.post('http://81.70.53.188:8080/uploadm41', formData);
