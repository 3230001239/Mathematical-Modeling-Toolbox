	...
	<div class="bg">
		<div class="m">
			...
			<div class="upload">
				...
				<button class="btn" @click="uploadFilem13">上传文件</button>
				<div style="color: red;">
					请上传csv文件
				</div>
				可点击此处下载测试数据csv文件-><a href="https://pan.baidu.com/s/1d7x48ldyLQPsKanxb6MTOQ?pwd=rcsv"
					target="_blank">点此下载</a>
				<h2>将在下方输出计算结果:</h2>
				<div v-if="uploadResult" style="width:470px;">
					<p><strong>十倍交叉验证的结果:</strong><br /> {{ uploadResult['out1'] }}</p>
					<p><strong>计算结果:</strong><br /> {{ uploadResult['out2'] }}</p>
				</div>
				<div v-if="uploadResult" style="width:420px;">
					<p><strong>计算结果:</strong><br /> {{ uploadResult['out3'] }}</p>
					<p><strong>计算结果:</strong><br /> {{ uploadResult['out4'] }}</p>
				</div>
	...
	export default {
		...
		methods: {
			...
			async uploadFilem13() {
				...
				try {
					const response = await axios.post('http://81.70.53.188:8080/uploadm13', formData, {
