	...
	<div class="bg">
		<div class="m">
			...
			<div class="upload">
				...
				<button class="btn" @click="uploadFilem51">上传文件</button>
				<div style="color: red;">
					请确保excel中只含有数字类型的数据,且文件为xlsx类型
				</div>
				可点击此处下载测试数据excel表格-><a href="https://pan.baidu.com/s/1wf_IsAWdextnrahPFSOGgg?pwd=szjf"
			...
			<div v-if="uploadResult">
				<p><strong>辛普森法求得结果:</strong> {{ uploadResult['output'] }}</p>
				<p><strong>龙贝格法求得结果:</strong> {{ uploadResult['output_two'] }}</p>
			</div>
	...
	export default {
		...
		methods: {
			...
			async uploadFilem51() {
				...
				// 发送文件到后端
				try {
					const response = await axios.post('http://81.70.53.188:8080/uploadm51', formData);
...
<style scoped>
	...
	.bg {
		height: 795px;
		background-image: url("images/m51d.png");
		display: flex;
	}
	.m {
		padding-left: 0px;
